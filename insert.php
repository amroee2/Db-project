...
// Check the connection
    ...
    if (isset($_POST['Sid'], $_POST['Pname'], $_POST['Amount'], $_POST['Pprice'])){
    $productName = $_POST['Pname'];
    $quantity = $_POST['Amount'];
    $purchasePrice = $_POST['Pprice'];
    $Sid = $_POST['Sid'];
    $sql="Insert into purchase_order_list(quantity, quantityXpurchasePrice, productName)
    values($quantity,$purchasePrice*$quantity,'$productName')";
    mysqli_query($conn,$sql);
    }
    if ($conn->connect_error) {
        die("Connection failed: " . $conn->connect_error);
        }
        $sql = "SELECT * FROM purchase_order_list";
    ...
    foreach ($data as $i) {
      ...
      <form target="dummyframe">
        <div class="centering">
          ...
          <strong class="amro">' . $i['productName'] . '</strong>
        </div>
        <strong class="amro"> ' . $i['quantity'] . 'x' . $i['quantityXpurchasePrice'] / $i['quantity'] . '</strong>
        <input type="hidden" name="orderListID" value="' . $i['PurchaseOrderList_id'] . '">
      </form>
      <script>
...
check2.addEventListener("click", function() {
    var xhr = new XMLHttpRequest();
    xhr.open("GET", "SupplierTrash.php", true);
    xhr.onload = function() {
        location.reload();
    };
    xhr.send();
